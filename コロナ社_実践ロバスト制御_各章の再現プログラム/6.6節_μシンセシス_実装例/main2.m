...
Kan=Ka;
%% パラメータ摂動の定義
scl=0.25; % 実数摂動を複素数領域の円で見積ってしまう分スケールをかける
...
[Kmu2,Gclp,mubnd,dkinfo]=dksyn(G,1,1,dkitopt);
%% 設計結果の確認
mkdir('図/設計2')
load('Kmu1.mat')
figure(2)
...
legend('設計1','設計2')
grid
saveas(gcf,'図/設計2/コントローラのボード線図')
% 以下の図で重みよりも外乱抑圧特性が下回ればおっけ
figure(3)
bodemag(1/Wps,'--',Ppert/(1+Ppert*Kmu2),w)
legend('1/Wps','P/(1+PK)')
grid
saveas(gcf,'図/設計2/ロバスト性能の確認')
figure(4)
bodemag(1/Wt,'--',Ppert*Kmu2/(1+Ppert*Kmu2),w);
legend('1/Wt','T(=PK/(1+PK))')
grid
saveas(gcf,'図/設計2/相補感度関数')
%% モデルマッチングをする
...
sum2=sumblk('e=yr-y');
%% 各種ブロックの接続
Gtdof=connect(Pn,Kmu2,Gff,M,sum1,sum2,'r',{'y','u'});
...
title('2自由度制御系の制御力')
grid
saveas(gcf,'図/設計2/ノッチフィルタありの2自由度制御の結果(ノミナル)')
%% ロバスト性能のチェック
load('Parray')
rng('default');
Ppert=Parray;
Ppert.y='y';
Ppert.u='u';
Gtdof=connect(Ppert,Kmu2,Gff,M,sum1,sum2,'r',{'y','u'});
figure(6)
subplot(211)
step(Gtdof(1,1),0.5);
ylim([0 1.2])
...
title('2自由度制御系の制御力')
grid
saveas(gcf,'図/設計2/ノッチフィルタありの2自由度制御の結果(ロバスト)')